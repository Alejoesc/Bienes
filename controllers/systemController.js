...
exports.exportBienes = async (req, res) => {
    try {
        ...
        workbook.creator = 'SGBN'; workbook.created = new Date();
        const worksheet = workbook.addWorksheet('Inventario de Bienes');
        worksheet.columns = [
            { header: 'Código', key: 'codigo_bien', width: 18 }, { header: 'Descripción', key: 'descripcion', width: 35 },
            { header: 'Serial', key: 'serial', width: 22 }, { header: 'Departamento', key: 'departamento', width: 25 },
            { header: 'Estado', key: 'estado', width: 15 }, { header: 'Valor ($)', key: 'valor', width: 15 },
        ...
        rows.forEach((item) => {
            worksheet.addRow({
                codigo_bien: item.codigo_bien || item.codigo || '', descripcion: item.descripcion || '', serial: item.serial || '',
                departamento: item.departamento || '', estado: item.estado || 'Activo', valor: Number(item.valor) || 0,
...
exports.importBienes = async (req, res) => {
    try {
        if (!req.file) return res.status(400).json({ error: "No Excel." });
        const workbook = new ExcelJS.Workbook(); await workbook.xlsx.readFile(req.file.path);
        const worksheet = workbook.worksheets[0];
        worksheet.eachRow(async (row, rowNumber) => {
            if (rowNumber > 1) {
                const codigo_bien = row.getCell(1).value; const descripcion = row.getCell(2).value;
...
exports.createBackup = async (req, res) => {
    try {
        ...
        res.setHeader('Content-Type', 'application/sql'); res.setHeader('Content-Disposition', `attachment; filename=sgbn_backup_${new Date().toISOString().split('T')[0]}.sql`); res.send(sqlDump);
    } catch (err) { res.status(500).json({ error: err.message }); }
};

exports.restoreBackup = async (req, res) => {
    try {
        if (!req.file) return res.status(400).json({ error: "No SQL." });
        ...
        for (let line of lines) {
            ...
            if (trimmed.endsWith(';')) {
                ...
                if (queryToRun.length > 0 && (queryToRun.toUpperCase().startsWith('INSERT') || queryToRun.toUpperCase().startsWith('TRUNCATE') || queryToRun.toUpperCase().startsWith('SET'))) {
                    try { await db.query(queryToRun); } catch (qErr) {}
                }
                currentQuery = '';
            }
        }
...
exports.getDepartamentos = async (req, res) => {
    try { const [rows] = await db.query("SELECT * FROM departamentos ORDER BY id ASC"); res.json(rows); } 
    catch (err) { res.status(500).json({ error: err.message }); }
};
