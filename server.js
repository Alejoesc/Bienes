...
async function verificarYCrearEstructura() {
    try {
        ...
        await db.query(`CREATE TABLE IF NOT EXISTS usuarios (id INT AUTO_INCREMENT PRIMARY KEY, username VARCHAR(100) UNIQUE NOT NULL, password VARCHAR(255) NOT NULL, role VARCHAR(50) DEFAULT 'viewer', nombre VARCHAR(100), departamento VARCHAR(100), cedula VARCHAR(30), correo VARCHAR(150)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_spanish_ci;`);
        await db.query(`CREATE TABLE IF NOT EXISTS bienes (id INT AUTO_INCREMENT PRIMARY KEY, codigo_bien VARCHAR(50) NOT NULL, codigo VARCHAR(50), descripcion TEXT NOT NULL, serial VARCHAR(100), departamento VARCHAR(100), estado VARCHAR(50) DEFAULT 'Activo', valor DECIMAL(12,2) DEFAULT 0.00, fecha_incorporacion DATE, fecha_desincorporacion DATE, fecha_enajenacion DATE, estatus VARCHAR(50) DEFAULT 'Activo') ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_spanish_ci;`);
...
// CORRECCIÓN AQUÍ: Cambiado a POST y ruta /api/desincorporar/:id
